...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        self.norm_spade = SPADE(output_nc, output_nc, param_free_norm_type="instance")
        model += [self.norm_spade]
        model += [nn.Tanh()]
        print("self.norm_spade")
    ...
    def forward(self, input, layers=[], encode_only=False):
        # 33
        # print("output length =", len(self.model)) 
        if -1 in layers:
...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc, param_free_norm_type='instance'):
        super().__init__()
