...
# This code borrowed from https://github.com/ydkim1293/NLNL-Negative-Learning-for-Noisy-Labels/blob/master/main_NL.py
class PatchNLLoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        labels_neg = (torch.LongTensor(out.size(0)).random_(1, out_patch).to(feat_q.device))

        # make sure neg_label [1, out_patch_size-1]
        assert labels_neg.max() <= out_patch - 1
        assert labels_neg.min() >= 1

        # loss = self.cross_entropy_loss(out, torch.zeros(out.size(0), dtype=torch.long,
        #                                                 device=feat_q.device))
        loss = self.criterion_nl(out, labels_neg)
        # print("nl loss =", loss)
        return loss
